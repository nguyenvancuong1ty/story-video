...
- LTX checkpoint: `ltxv-2b-0.9.8-distilled.safetensors`; T5 was converted locally to a single BF16 file for ComfyUI reuse.
- Direct T2V benchmark: 121 frames, 768x448, 24 fps, ~18-27s after warmup; visual identity was not reliable enough across independent shots.
- I2V test with one persistent protagonist reference: three 5s variants completed in ~19-21s each. Single-start-reference conditioning improved continuity but still allowed end-of-shot face drift and one extra background person.
- Production preset changed to dual `LTXVAddGuide`: start strength 2.0 + end strength 1.25, then `LTXVCropGuides` before decode. This kept face/hair/outfit more stable and removed the extra-person failure in the tested shot.
- Reusable provider added: `scripts/ltx-comfy-provider.py`.
- `render-second-act-openmontage.py` now accepts backward-compatible `ai_generate` objects inside beat `clips`; it materializes LTX assets before OpenMontage composition.
- Provider smoke test PASS: 121 frames / 5.0417s / 768x448 / 24fps, ~23.2s total.
- Hybrid integration draft PASS: 35.07s, 1920x1080, 30fps, H.264, audio/subtitles present, OpenMontage final_review PASS. First shot was generated by LTX and remaining shots were stock.
- Semantic QA still matters: the existing stock pilot plan contains at least one older male shot that conflicts with the female protagonist. Technical final_review does not currently detect this; visual-direction review must reject wrong-gender protagonist inference.
- Current safe usage: AI for subtle protagonist hero/anchor shots in the same or closely related environment; stock for broader scene variety. Cross-environment identity preservation remains a future IC-LoRA/reference-routing upgrade.

## 21. Identity keyframe + hybrid v3 - 2026-08-28

- Confirmed official LTX 0.9.8 IC-LoRA detailer is not the generic face-identity path for the current 2B stack.
- Production identity route is now: `master portrait -> PhotoMaker SDXL scene keyframe -> LTX I2V -> normalize -> OpenMontage`.
- Native ComfyUI PhotoMaker nodes are available; local models: `sd_xl_base_1.0.safetensors` (~6.94GB) and `photomaker-v1.bin` (~934MB).
- Added `scripts/photomaker-keyframe-provider.py`; warm keyframe generation tested around 8s, with some first-run cases ~17s.
- PhotoMaker native node requires concise prompts; provider now rejects prompts over 60 words to avoid the 154-vs-77 token mask failure.
- Tested multiple identity seeds; `office-73113.png` was the best match for the current protagonist reference.
- LTX animated that keyframe to 121 frames / 5.04s; identity stayed stable across sampled frames with no extra person or obvious hand failure.
- AI footage normalizer outputs 1920x1080 before composition using restrained upscale/sharpen/grain matching.
- Semantic QA now supports clip roles and rejects unapproved stock used as `role: protagonist`.
...
## 24. Vietnamese production-test pilot - 2026-08-28

- Added `scripts/prepare-second-act-vietnamese-pilot.py` to localize a source run through the configured local LLM, synthesize Vietnamese CapCut narration, make a separate run directory, and write a strict semantic plan without changing the English source run.
- CapCut Vietnamese voice index 4 was listed by the service but rejected by its backend (`TTSInvalidSpeaker`); the tested, working voice is index 0 (`BV421_vivn_streaming`). Long narration is split by sentence and rejoined per beat so the local TTS service can resume safely.
- The continuity plan also replaces the remaining male-hands Medicare shot (`pexels_8439245`) with neutral paperwork (`pexels_7055354`).
- Accepted Vietnamese test render: `out/second-act/divorced-at-63-with-no-retirement-how-i-rebuilt-my-life-from-vi-pilot-rerun/second-act-openmontage-vi-production-test-v2.mp4`.
- V2 passed strict semantic QA and OpenMontage final review: 227.44s (3:47), 1920x1080, 30fps, H.264 + AAC, 45 cuts, and subtitle coverage 1.0. Visual sampling confirmed the Medicare replacement has no visible male continuity cue.
- This is a production-path test, not yet a 5-minute cut. To reach five minutes, expand the Vietnamese script/narration before synthesizing audio; do not time-stretch the current voice track.
